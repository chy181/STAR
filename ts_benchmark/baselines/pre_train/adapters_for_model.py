...
class PreTrainAdapter(ModelBase):
    ...
    def detect_fit(self, train_data: pd.DataFrame, test_data: pd.DataFrame):
        ...
        if config.is_train:
            ...
            for epoch in range(config.num_epochs):
                self.model.train()
                for i, (continuous, label) in enumerate(self.train_data_loader):
                    optimizer.zero_grad()
                    input = continuous.float().to(self.device)
    ...
    def detect_label(self, test: pd.DataFrame) -> np.ndarray:
        ...
        save_name = f'checkpoints/test11.pt'
    ...
    #             rec_c = self.model(continuous, discrete)
    #             reconstruct_scores = self.anomaly_criterion(continuous,rec_c).squeeze(-1) 
    #             attens_rec.append(reconstruct_scores.detach().cpu())
    #             test_labels.append(label)
    #             test_labels.append(label)

    #         attens_rec = torch.cat(attens_rec, dim=0).reshape([-1])
    #         attens_rec = attens_rec.numpy()
    #         train_energy_rec = np.array(attens_rec)
    ...
    #             discrete = discrete.float().to(self.device)
    #             rec_c = self.model(continuous, discrete)
    #             reconstruct_scores = self.anomaly_criterion(continuous,rec_c).squeeze(-1) #+ torch.mean(self.freq_anomaly_criterion(continuous ,result_dict['rec_c']), dim=-1) * self.config.score_lambda
    #             attens_rec.append(reconstruct_scores.detach().cpu())
    #             test_labels.append(label)
    #             test_labels.append(label)
    ...
    #         attens_rec = torch.cat(attens_rec, dim=0).reshape([-1])
    #         attens_rec = attens_rec.numpy()
    #         # attens_energy = torch.softmax(torch.log(torch.tensor(attens_rec)),dim=0).numpy() *  attens_sim#attens_rec#*
    #         test_energy = np.array(attens_rec)

    #         combined_energy = np.concatenate([train_energy_rec, test_energy_rec], axis=0)
    #         test_energy = combined_energy[-test_energy_sim.shape[0]:]
