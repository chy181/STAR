...
class SeqAttention(nn.Module):
    ...
    def forward(self, x, attn_mask=None, n_vars=0):
        B, N, C = x.shape
        x = rearrange(x,'(b n) p d -> b n p d', n=n_vars)
        qkv = self.qkv(x)
        qkv = rearrange(qkv,'b n p d -> (b n) p d')
...
class SeqAttBlock(nn.Module):
    ...
    def forward(self, x, attn_mask):
        ...
        x = self.attn_seq(x, attn_mask, n_vars=n_vars)
        x = torch.reshape(
...
class Model(nn.Module):
    ...
    def anomaly_detection(self,  x, discrete, task_id):
        ...
        num_patch = x.size(1)
        # # plugin
        # b, n_disc, p, d = discrete.shape
        # x = rearrange(x,'(b n) p d -> b n p d',n=n_vars)
        # x = torch.cat([x,discrete],dim=1)
        # x = rearrange(x,'b n p d -> (b n) p d')
        # n_vars = n_vars + n_disc
        # 
...
class UniTS(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.missing_keys = msg.missing_keys
    ...
    def forward(self, continuous, discrete=None): 
        point_forecast, result_dict = self.model.anomaly_detection(continuous,discrete, 0) 
        return point_forecast, result_dict