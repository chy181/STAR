...
class Linear_extractor_cluster(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.noisy_gating = True #config.noisy_gating #=============== 是否需要noisy_gating True or False
        self.num_experts = config.num_experts #==============专家个数 其实就是nn.Embedding个数 词表的数量
        self.input_size =  config.seq_len #==============输入长度
        self.k = 2 #config.k  #==================最后选择专家的个数 就是我们从N个词表中最后选择K个
        # instantiate experts
        config.hidden_size=256
        self.experts = nn.Embedding(self.num_experts+1,32)#nn.ModuleList([expert(config) for _ in range(self.num_experts)]) #================= 专家 改成我们的词表用nn.Embedding[]代替
        self.W_h = nn.Parameter(torch.eye(self.num_experts)) 
        self.gate = encoder(config)  #=================config里要包含：seq_len num_experts hidden_size
        self.noise = encoder(config) #=================config里要包含：seq_len num_experts hidden_size

        self.n_vars = config.enc_in
        #self.revin = RevIN(self.n_vars) #=================我们不要revin了 因为都是离散变量
        #self.CI = config.CI    #=================不要revin 这个参数就没用了
        self.softplus = nn.Softplus()
        self.softmax = nn.Softmax(1)
        self.register_buffer("mean", torch.tensor([0.0])) #=================不要revin 这个参数就没用了
        self.register_buffer("std", torch.tensor([1.0])) #=================不要revin 这个参数就没用了
    ...
    def forward(self, x, loss_coef=1):
        #================= 输入离散变量 输出离散变量的嵌入+loss（这个loss的作用是确保每个专家都不偷懒，这个loss要去改 adapter里的输出 确保他被优化上 一般他*0.001再加上本来的loss）
        """Args:
       ...
       # gates = dispatcher.expert_to_gates()
        ...
        #     print(f"expert[{i}] input shape:", ei.shape)
        # expert_outputs = [
        #     self.experts[i](expert_inputs[i]) for i in range(self.num_experts)
        # ]
        expert_outputs=[]
        # for i in range(self.num_experts):
        #     vector = self.experts[i]
        #     vector.unsqueeze(0).expand(expert_inputs[i].shape[0],-1)
        #     expert_outputs.append(vector)
        for i in range(self.num_experts):
            input_ids = torch.full((expert_inputs[i].shape[0],), i, dtype=torch.long, device=x.device)
            vector = self.experts(input_ids)  # [batch_i, embedding_dim]
            expert_outputs.append(vector)
